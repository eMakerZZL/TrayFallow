...
px_move = sx_move ./ s_move;
py_move = sy_move ./ s_move;
pz_move = sz_move ./ s_move;
...
% 
% v_move = vx_move ./ px_move(1, end - 1);
% vy_move = v_move .* py_move(1, end - 1);
% vz_move = v_move .* pz_move(1, end - 1);
...
figure(2);
plot3(sx_stable,sy_stable,sz_stable,sx_move,sy_move,sz_move);title('位移路径对比');grid on;
hold off;

figure(3);
diff_sx = sx_move - sx_stable;
diff_sy = sy_move - sy_stable;
diff_sz = sz_move - sz_stable;
plot(diff_sx,'r');hold on;
plot(diff_sy,'g');
plot(diff_sz,'b');
hold off;